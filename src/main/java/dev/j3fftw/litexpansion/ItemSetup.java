...
final class ItemSetup {
  ...
  public ItemSetup(SlimefunAddon plugin) {
    this.plugin = plugin;
  }
  ...
  private void registerRubber() {
    if (Bukkit.getPluginManager().isPluginEnabled("SlimyTreeTaps")) {
      this.rubberItem = Objects.requireNonNull(SlimefunItem.getById("RUBBER")).getItem();
    } else if (Bukkit.getPluginManager().isPluginEnabled("SlimeTech")) {
      this.rubberItem = Objects.requireNonNull(SlimefunItem.getById("SLIMETECH_RUBBER")).getItem();
    } else {
  ...
  private void registerMiscItems() {
    ...
    registerNonPlaceableItem(
        Items.UNINSULATED_TIN_CABLE, 3, ManualMill.RECIPE_TYPE, Items.TIN_ITEM_CASING);
    ...
    registerNonPlaceableItem(
        Items.UNINSULATED_COPPER_CABLE, 3, ManualMill.RECIPE_TYPE, Items.COPPER_ITEM_CASING);
    ...
    registerNonPlaceableItem(
        Items.UNINSULATED_GOLD_CABLE, 3, ManualMill.RECIPE_TYPE, Items.GOLD_ITEM_CASING);
    ...
    RecipeType.SMELTERY.register(
        new ItemStack[] {Items.LAPIS_DUST}, new ItemStack(Material.LAPIS_LAZULI));
    RecipeType.SMELTERY.register(
        new ItemStack[] {Items.REDSTONE_DUST}, new ItemStack(Material.REDSTONE));
    RecipeType.SMELTERY.register(
        new ItemStack[] {Items.DIAMOND_DUST}, new ItemStack(Material.DIAMOND));
    RecipeType.SMELTERY.register(
        new ItemStack[] {Items.EMERALD_DUST}, new ItemStack(Material.EMERALD));
    RecipeType.SMELTERY.register(
        new ItemStack[] {Items.QUARTZ_DUST}, new ItemStack(Material.QUARTZ));
    RecipeType.SMELTERY.register(
        new ItemStack[] {Items.ANCIENT_DEBRIS_DUST}, new ItemStack(Material.NETHERITE_INGOT));
  ...
  private void registerEndgameItems() {
    registerNonPlaceableItem(
        ...
        new CustomItemStack(Material.COBBLESTONE, "&7Any Item!"));
    new DyeItem(
            Items.LITEXPANSION,
            Items.UU_MATTER,
            MassFabricator.RECIPE_TYPE,
            createSingleItemRecipeCentered(Items.SCRAP))
        .register(plugin);
    new DyeItem(
            Items.LITEXPANSION,
            Items.IRIDIUM,
  ...
  private void registerCarbonStuff() {
    new DyeItem(
            ...
            createSingleItemRecipe(new ItemStack(Material.COAL)))
        .register(plugin);
    new DyeItem(
            Items.LITEXPANSION,
  ...
  private void registerNonPlaceableItem(
      @Nonnull SlimefunItemStack item, @Nonnull RecipeType type, @Nonnull ItemStack... items) {
  ...
  private void registerNonPlaceableItem(
    ...
    new UnplaceableBlock(
            Items.LITEXPANSION, item, type, recipe, new SlimefunItemStack(item, amount))
        .register(plugin);
  }

  /**
   * Builds and ItemStack array that centers the item in the guide. Only to be used for display
   * recipes that have no effect, i.e. UU Matter in the Mass Fabricator
   *
   * @param item is the item shown in the guide
   * @return a prebuilt recipe ItemStack array
   */
  private ItemStack @NotNull [] createSingleItemRecipeCentered(ItemStack item) {
    return new ItemStack[] {null, null, null, null, item, null, null, null, null};
  }

  /**
   * Builds an ItemStack array for shapeless recipe machines, like the {@link OreCrusher}
   *
   * @param item is the item shown in the guide
   * @return a prebuilt recipe ItemStack array
   */
  private ItemStack @NotNull [] createSingleItemRecipe(ItemStack item) {
