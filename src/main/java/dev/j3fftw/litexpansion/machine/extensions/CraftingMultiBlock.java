...
public abstract class CraftingMultiBlock extends MultiBlockMachine {
  ...
  protected CraftingMultiBlock(
      @NotNull ItemGroup itemGroup,
      @NotNull SlimefunItemStack item,
      ItemStack @NotNull [] recipe,
      ItemStack @NotNull [] machineRecipes,
      @NotNull BlockFace trigger) {
    super(itemGroup, item, recipe, machineRecipes, trigger);
  }

  protected @NotNull Inventory createVirtualInventory(@NotNull Inventory inv) {
    Inventory fakeInv = Bukkit.createInventory(null, 9, "Fake Inventory");

    for (int j = 0; j < inv.getContents().length; j++) {
      ItemStack stack =
          inv.getContents()[j] != null && inv.getContents()[j].getAmount() > 1
              ? new CustomItemStack(inv.getContents()[j], inv.getContents()[j].getAmount() - 1)
              : null;
    ...
    if (outputInv != null) {
      ...
      if (removeSpecialBlock()) {
        ...
        if (specialBlock != null) {
          ...
          specialBlock
              .getWorld()
              .spawnParticle(Particle.PORTAL, specialBlock.getLocation(), 4, 0.5, 0.5, 0.5);
        }
      }
    } else {
  ...
  private boolean isCraftable(@NotNull Inventory inv, ItemStack[] recipe) {
    for (int j = 0; j < inv.getContents().length; j++) {
